...
@app.route('/upravljaj_clanovima', methods=['GET'])
def upravljaj_clanovima():
    query = request.args.get('query')
    try:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        if query:
            cursor.execute('SELECT * FROM clan WHERE prezime LIKE %s', (f'%{query}%',))
        else:
            cursor.execute('SELECT * FROM clan')
        clanovi = cursor.fetchall()
    except MySQLdb.Error as e:
        flash(f'Greška prilikom dohvata članova: {e}', 'danger')
        clanovi = []
    finally:
        cursor.close()
    return render_template('upravljaj_clanovima.html', clanovi=clanovi, query=query)
